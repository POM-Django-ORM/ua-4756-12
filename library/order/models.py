...
class Order(models.Model):
    ...
    book = models.ForeignKey("book.Book", on_delete=models.CASCADE, related_name="orders")
    ...
    def __str__(self):
        ...
        return (
            f"'id': {self.id}, 'user': {self.user!r}, 'book': {self.book!r}, "
    ...
    def to_dict(self):
        ...
        return {
            ...
            "end_at": self.end_at.timestamp() if self.end_at is not None else None,
    ...
    def create(user, book, plated_end_at):
        ...
        if user.id is None or book.id is None:
            return None
        not_returned_orders_count = Order.objects.filter(book=book, end_at__isnull=True).count()
        if not_returned_orders_count >= book.count:
            return None
